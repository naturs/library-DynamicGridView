...
public class DynamicGridView extends GridView {
	...
	static final String LOG_TAG = DynamicGridView.class.getSimpleName();
	...
	private OnItemClickListener mLocalItemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			// 在编辑模式中，点击某一个item，会隐藏该item，显示的是它的screenshot，所以此时item点击事件无效
	...
	private Bitmap getBitmapFromView(View v) {
		...
		canvas.drawLine(0, 0, v.getWidth(), v.getHeight(), paint);
		canvas.drawLine(0, v.getHeight(), v.getWidth(), 0, paint);
		
		return bitmap;
	}

	/**
	 * 拿到我们点击位置的view之外的其他view
	 * @param itemId
	 */
	private void updateNeighborViewsForId(long itemId) {
		...
		// ID -> VIEW -> POSITION
		// getAdapter().getItemId(position) => POSITION -> ID
		int draggedPos = getPositionById(itemId);
	 ...
	 * @return
	 */
	public View getViewById(long itemId) {
	...
	private void touchEventsEnded() {
		final View mobileView = getViewById(mMobileItemId);
	...
	private void touchEventsCancelled() {
		View mobileView = getViewById(mMobileItemId);
		if (mCellIsMobile) {
			reset(mobileView);
		}
		mCellIsMobile = false;
	...
	/**
	 * 处理item view之间的交换。其实并不是view做了交换，只是两个位置的数据做了交换，然后调用notifyDataSetChanged方法。
	 * 
	 * 注意这里交换的规则，是根据mobileView和周围view的位置来计算的。而周围的view在GridView滚动过程中会发生改变，
	 * 根据firstVisiblePosition ~ lastVisiblePosition，得到新的周围的view。
	 */
	private void handleCellSwitch() {
		final int deltaY = mLastEventY - mDownY;
		final int deltaX = mLastEventX - mDownX;
		
		// 这里改名为centerX和centerY更好，代表screenshot中心点的坐标。原来的中心点坐标+delta=新的中心点坐标
		// 这里完全可以不使用mTotalOffsetX和mTotalOffsetY两个值，使用它们的原因是在该方法中，有mDownX = mLastEventX
		// 和mDownY = mLastEventY两个操作，如果去掉这两个操作，并且不使用mTotalOffsetX和mTotalOffsetY，对结果没影响。
		...
		for (Long id : idList) {
			...
			if (view != null) {
				...
				L.d(LOG_TAG, "id:%d, %d x %d.", id, targetColumnRowPair.y, targetColumnRowPair.x);
				
				/* 右上面的view，中心点超过view的左下角坐标即可*/
				boolean aboveRight = aboveRight(targetColumnRowPair, mobileColumnRowPair) && deltaYTotal < view.getBottom() && deltaXTotal > view.getLeft();
				boolean aboveLeft = aboveLeft(targetColumnRowPair, mobileColumnRowPair) && deltaYTotal < view.getBottom() && deltaXTotal < view.getRight();
				boolean belowRight = belowRight(targetColumnRowPair, mobileColumnRowPair) && deltaYTotal > view.getTop() && deltaXTotal > view.getLeft();
				boolean belowLeft = belowLeft(targetColumnRowPair, mobileColumnRowPair) && deltaYTotal > view.getTop() && deltaXTotal < view.getRight();
				
				// 这里mOverlapIfSwitchStraightLine的作用：比如view向右上拖动的时候，中心点位置很容易超过上面view的底部，但此时还未
				// 超过右上角view的左部，导致此时view和上面的view做了交换。这个值就是一个缓冲的作用，使效果更好。
				/* 上面的view，中心点超过view的底部坐标即可 */
				boolean above = above(targetColumnRowPair, mobileColumnRowPair) && deltaYTotal < view.getBottom() - mOverlapIfSwitchStraightLine;
				boolean below = below(targetColumnRowPair, mobileColumnRowPair) && deltaYTotal > view.getTop() + mOverlapIfSwitchStraightLine;
				boolean right = right(targetColumnRowPair, mobileColumnRowPair) && deltaXTotal > view.getLeft() + mOverlapIfSwitchStraightLine;
				boolean left = left(targetColumnRowPair, mobileColumnRowPair) && deltaXTotal < view.getRight() - mOverlapIfSwitchStraightLine;
				
				
				if (aboveRight || aboveLeft || belowRight || belowLeft || above || below || right || left) {
					L.d(LOG_TAG, "aboveRight:%s, aboveLeft:%s, belowRight:%s, belowLeft:%s, above:%s, below:%s, right:%s, left:%s.", aboveRight, aboveLeft, belowRight,  belowLeft, above, below, right, left);
		...
		if (targetView != null) {
			...
			L.d(LOG_TAG, "targetPosition=%d.", targetPosition);
			...
			// reorder后，mobileView显示的是新的数据，它原来是INVISIBLE的，要让它显示出来
			mobileView.setVisibility(View.VISIBLE);
			// reorder后，targetView显示的是原来mobileView位置的数据，但原来mobileView的screenhot还处于
			// 拖动状态，这时候就不能让它显示出来。
			if (isPostHoneycomb()) {
			...
			// 这里如果是将mobileView插入到targetPosion位置，使用动画效果会比较好。
			// 如果是两个位置交换，不要动画效果会比较好。
	...
	/**
	 * 获取view在GridView中的几行几列（下标从0开始）
	 * @param view
	 * @return 列x行
	 */
	private Point getColumnAndRowForView(View view) {
		int pos = getPositionForView(view);
		int columns = getColumnCount(); // GridView列数
		int column = pos % columns; // 列数下标
		int row = pos / columns; // 行数下标
	...
	private void animateReorder(final int oldPosition, final int newPosition) {
		...
		if (isForward) {
			for (int pos = Math.min(oldPosition, newPosition); pos < Math.max(oldPosition, newPosition); pos++) {
				View view = getViewById(getId(pos));
